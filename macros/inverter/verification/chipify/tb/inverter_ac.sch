...
only_toplevel=true 
value="
* Swept by chipify. Braces are xschem delimiters, so every Jinja
* placeholder below is escaped in the .sch source and renders unescaped.
.param VDD=\{\{ vdd \}\}
.param Vcm=VDD/2
.param Cload=\{\{ cload \}\}
.temp \{\{ temp \}\}
...
* Every name below is a bare vector, which is what chipify can capture.
...
* chipify splices its own echo MY_DATA: line in before this quit.
quit
.endc
"}
